...
function ReturnDetailPage() {
  ...
  return (
    <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        ...
        <Descriptions column={{ xs: 1, sm: 2 }} bordered>
          ...
          <Descriptions.Item label="Сумма возврата" span={2}>
            <Text strong style={{ fontSize: 20, color: '#1890ff' }}>
              {/* ИСПРАВЛЕНО: заменено с ret.total_amount на ret.refund_amount */}
      ...
      <Title level={4}>Товары ({ret.items ? ret.items.length : 1})</Title>
      {/* В новой логике возврат создается на один конкретный товар (OrderItem),
          поэтому ret.items может отсутствовать, а данные берутся из самого ret */}
      <List
        dataSource={ret.items || [ret]}
        renderItem={item => {
          // Адаптация под структуру, где товар может быть вложенным или плоским
          const itemName = item.product_name || 'Товар по заявке';
          const colorName = item.color_name || '';
          const sizeLabel = item.size_label || '';
          const qty = item.quantity || ret.quantity;
          const amount = item.refund_amount || ret.refund_amount;
              ...
              <List.Item.Meta
                ...
                description={
                  <Space direction="vertical" size={0}>
                    {(colorName || sizeLabel) && <Text>{colorName} {colorName && sizeLabel ? '•' : ''} {sizeLabel}</Text>}
                    <Text>Количество: {qty} шт.</Text>
                  </Space>
                }
              />
