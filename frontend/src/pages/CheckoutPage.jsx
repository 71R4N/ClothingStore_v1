...
function CheckoutPage() {
    ...
    const onFinish = async (values) => {
        ...
        try {
            ...
            // 2. Инициируем платеж
            try {
                const paymentRes = await paymentService.initiatePayment(orderId);
                const confirmationUrl = paymentRes.data.confirmation_url;

                message.success('Заказ создан! Перенаправляем на оплату...');

                // 3. Редирект на страницу оплаты ЮKassa
                setTimeout(() => {
                ...
                message.warning(
                    'Заказ создан, но не удалось инициировать платеж. ' +
                    'Перейдите в раздел заказов для оплаты.'
                );
                navigate(`/orders`);
            }
            ...
            console.error(e);
            setError(
                e.response?.data?.detail || "Ошибка при создании заказа"
            );
        } finally {
            setLoading(false);
        }
